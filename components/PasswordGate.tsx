...
const PasswordGate: React.FC<PasswordGateProps> = ({ 
  ...
  useEffect(() => {
    ...
    const authStatus = localStorage.getItem('cbre_analytics_auth');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (inputPassword === password) {
      ...
      localStorage.setItem('cbre_analytics_auth', 'authenticated');
      setIsAuthenticated(true);
    } else {
  ...
  const handleLogout = () => {
    localStorage.removeItem('cbre_analytics_auth');
      ...
      <div className="min-h-screen flex items-center justify-center bg-custom-cream px-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8">
          ...
          <div className="mt-6 space-y-3">
            ...
            <a
              href={`mailto:John.Luke8054@gmail.com?subject=${encodeURIComponent('Password Request - CBRE Analytics Engine')}&body=${encodeURIComponent('Hi John,\n\nI would like to request access to the CBRE Analytics Engine project. Could you please share the password?\n\nThank you!')}`}
